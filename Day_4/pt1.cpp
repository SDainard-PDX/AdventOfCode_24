#include <iostream>
#include <string>
#include <fstream>
#include <vector>


#define FILE_IN "../t1data.txt"
using namespace std;


int findXMAS(const vector<vector<char>> &field, pair<int,int> dir, pair<int,int> pos ) {
    if (field[pos.second] [pos.first] == 'X') {
        //cout << "(" << pos.second + 1 << "," << pos.first + 1 << ")X ";
        if (field[pos.second + dir.second][pos.first + dir.first] == 'M') {
        //    cout << "(" << pos.second + dir.second + 1 << "," << pos.second + dir.second + 1 << ")M ";
            if (field[pos.second + dir.second*2][pos.first + dir.first*2] == 'A') {
        //        cout << "(" << pos.second + dir.second*2 + 1 << "," << pos.second + dir.second*2 + 1 << ")A ";
                if (field[pos.second + dir.second*3][pos.first + dir.first*3] == 'S') {
        //            cout << "(" << pos.second + dir.second*3 + 1 << "," << pos.first + dir.first*3 + 1 << ")S "
        //            <<  "\tHIT" << endl;
                    return 1;
                }
            }
        }
        //cout << endl;
    }
    return 0;
}


int main() {
    ifstream in_file(FILE_IN);
    if (!in_file.is_open()) {
        cout << "failed to open file" << endl;
        return EXIT_FAILURE;
    }

    //variables
    int verb = 0, total = 0;
    char in;
    vector<vector<char>> field = {};
    vector<char> line = {};

// create a field of input using a double vector
    while (in_file.get(in)) {
        if (in == '\n') {
            field.push_back(line);
            line = {};
        }
        else line.push_back(in);
    }
    field.push_back(line);
    in_file.close();

// for printing the field
    if(verb) {
        for (auto x: field) {
            for (auto y: x) cout << y;
            cout << endl;
        }
    }

// find all the x's and check each of the 9 directions
    int row = 0, col = 0;
    int row_max = line.size(), col_max = field.size();

    for (auto y: field) {
        row = 0;
        for (char x: y) {
            bool left = false, up = false, right = false, down = false;
            if (x == 'X') {
                if (row > 2) left = true;
                if (col > 2) up = true;
                if (row < (row_max - 3)) right = true;
                if (col < (col_max - 3)) down = true;

                if (left) {
                    if(verb) cout << "left ";
                    total += findXMAS(field, {-1, 0}, {row, col}); }
                if (left && up) {
                    if(verb) cout << "left & up ";
                    total += findXMAS(field, {-1, -1}, {row, col}); }
                if (up) {
                    if(verb) cout << "up ";
                    total += findXMAS(field, {0, -1}, {row, col}); }
                if (up && right) {
                    if(verb) cout << "up & right ";
                    total += findXMAS(field, {1, -1}, {row, col}); }
                if (right) {
                    if(verb) cout << "right ";
                    total += findXMAS(field, {1, 0}, {row, col}); }
                if (right && down) {
                    if(verb) cout << "right & down ";
                    total += findXMAS(field, {1, 1}, {row, col}); }
                if (down) {
                    if(verb) cout << "down ";
                    total += findXMAS(field, {0, 1}, {row, col}); }
                if (down && left) {
                    if(verb) cout << "down & left ";
                    total += findXMAS(field, {-1, 1}, {row, col}); }
            }

            ++row;
        }
        //end of line here
        ++col;
    }

    cout << "\n\t Tot = " << total << endl;
    return EXIT_SUCCESS;
}
