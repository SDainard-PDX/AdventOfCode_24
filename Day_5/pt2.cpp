...
int main() {
    ...
    if (verb) {
        cout << "Leads rules" << "\tFollows rules" << endl;
        auto rule_f = follows.begin();
        for (auto rule_l : leads) {
            cout << rule_l.first << " - " << rule_l.second << "\t\t";
            cout << rule_f->first << " - " << rule_f->second << endl;
            ++rule_f;
        }
        cout << endl;
    }
    ...
    while ( !in_file.eof() ) {
        ...
        while (!good) {
            ...
            for (int page = 0; page < n - 1; ++page) {
                ...
                if (range_f.first !=
                    range_f.second) {                                //if it exists go over all elem in value
                    for (int following = page + 1; following < n && good; ++following) {   // comparing prior pages
                        for (auto it = range_f.first; it != range_f.second; ++it) {
                            if (verb > 1)
                                cout << "Page " << print_order[page] <<
                                     " follows " << it->second << " compare to " << print_order[following];
                            if (print_order[following] == it->second) {
                                auto swa = find(print_order.begin(), print_order.end(), print_order[following]);
                                int idx = distance(print_order.begin(), swa);
                                swap(print_order[page], print_order[idx]);
                                if (verb) {
                                    cout << "\tbad1" << endl;
                                    for (auto blah: print_order) cout << blah << ", ";
                                }
                                good = false;
                                fixed = true;
                                break;
...
*/
        ...
        if (!fixed) { if(verb) cout << "\tGood, ignore" << endl; } // do nothing
        else {
            total += print_order[n/2];
            if (verb) cout << endl << "\tNow good mid is " << print_order[n/2] << " total is " << total << endl;
        }
    }
